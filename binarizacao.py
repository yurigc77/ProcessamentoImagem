...
def otsu(img):
    ...
    hist = histogramaOtsu(img)#faz o histograma da imagem
...
def main():
    ...
    gray=cv.cvtColor(img,cv.COLOR_BGR2GRAY) #imagem cinza
    cv.imshow('Escala de cinza',gray)


    (thresh, bin) = cv.threshold(gray, otsu(gray), 255, cv.THRESH_BINARY) #binarizando com otsu como limite
    cv.imshow('Binarizada Otsu', bin)


    (thresh, bin) = cv.threshold(gray, 120, 255, cv.THRESH_BINARY)#binarização com limite fixo para comparar
